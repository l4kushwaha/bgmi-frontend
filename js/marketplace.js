(() => {
  ...
  function renderCard(item) {
    ...
    card.innerHTML = `
      ...
      <div class="verified-badge">${item.seller_verified == 1 ? "Verified" : "Pending"}</div>
      ...
      ${isOwner ? `<button class="btn edit-btn">Edit</button>` : ""}
    `;

    if (isOwner) card.querySelector(".edit-btn").onclick = () => openEdit(item);
  ...
  document.getElementById("save-edit").onclick = async () => {
    ...
    const images = [...document.querySelectorAll("#e-images img")].map(i => i.src);
